...
const {expand} = require("../phonetification");
const enumElemType = {
	node: 0,
	way: 1,
	relation: 2
...
class StreetAssembler{
	...
	_getTotalSubdivCount(subdivs = this.subDivisions){
		let subdivCount = 0;
		for(let i = 0; i < subdivs.length; i += 1){
			/**@type {Array} */
			const subsubsdivs = subdivs[sSubDivision];
			if(subsubsdivs && subsubsdivs.length){
				subdivCount += this._getTotalSubdivCount(subsubsdivs);
			}
		}
