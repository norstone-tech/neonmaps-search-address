...
class StreetAssembler{
			...
			for(let i = 0; i < mapThings.length; i += 1){
				...
				if(place.tags.has("addr:interpolation") && place.type == "way"){
					...
					if(!streetData.streetFullName){
						/* TODO: This only happens because this tool doesn't handle "merged" address interpolators well
						   Which seperate their house numbers and streets using semicolons. */
	...
	_createProtoSubdivs(subdivs = this.subDivisions){
		...
		for(let i = 0; i < subdivs.length; i += 1){
			...
			result.push({
				...
				lat: Math.round(centroid.geometry.coordinates[1] * 1000000000),
				lon: Math.round(centroid.geometry.coordinates[0] * 1000000000),
	...
	async createFile(){
		...
		const magic = "neonmaps.addresses\x01";
