...
class StreetAssembler{
	...
	getStreetType(streetName, principalSubdivCode){
		...
		// This line is overly complicated, but at leas
		const {
			streetTypePlacement: streetTypePlacementList
		} = Object.assign(
		...
		for(let i = 0; i < streetTypePlacementList.length; i += 1){
			...
			switch (streetTypePlacementList[i]){
				case "before":{
					...
					for(let i = 0; i < this.countryRules.streetTypes.length; i += 1){
		...
		if(!streetType){
			for(let i = 0; i < this.countryRules.streetTypeWhitelist.length; i += 1){
				if(this.countryRules.streetTypeWhitelist[i].test(streetName)){
					return [null, streetDirection, streetName, null];
				}
			}
		}
