...
class StreetAssembler{
	...
	_flattenProtoSubdivs(
		...
		for(let i = 0; i < protoSubdivs.length; i += 1){
			...
			result.set(curPath + protoSubdiv.name, protoSubdiv);
			this._flattenProtoSubdivs(
				protoSubdiv.subdivision,
				curPath ? (curPath + path.sep + protoSubdiv.name) : protoSubdiv.name,
				result
			);
		}
		return result;
		...
		for(const [subdivPath, subdiv] of flatProtoSubdivs){
			...
			for(let i = 0; i < streetFileNames.length; i += 1){
				...
				const protoAddr = StreetAddressParser.read(new Pbf(
					await fsp.readFile(path.resolve(this.dir, subdivPath, streetFileNames + ".pbf"))
				));
				...
				for(let i = 0; i < interpolations.length; i += 1){
					const interWay = await this.mapReader.getWay(interpolations[i]);
				...
				const fuzzedName = phonetify(protoAddr.streetPhoneticName, subdivLang, true, false);
				let fuzzedNum = fuzzedName.substring(1);
				...
				protoAddr[sStreetGroupIndex] = fuzzedName.charCodeAt(0);
				protoAddr[sStreetIndexNum] = Number(fuzzedNum);
				...
				const lastInterpolation = interpolations[0];
			...
			if(streets.length){
				streets.sort((a, b) => {
					const groupIndexDiff = a[sStreetGroupIndex] - b[sStreetGroupIndex];
					if(groupIndexDiff != 0){
						return groupIndexDiff;
					}
					return a[sStreetIndexNum] - b[sStreetIndexNum];
				});
